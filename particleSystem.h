...
public:
	...
	/** Constructor **/
	ParticleSystem(force** f, int n_f);
	...
	//set fps
	void setBakeFps(const float& fps) { bake_fps = fps; }
	//check if there is a set of baked particles at time t.
	bool isBaked(const float& t);
	//get baked particles
	void getBakedParticles(const float& t);
	//append particle to "particles" vector
	void AddParticleStartingAt(Vec3f WorldPoint, Vec3f colour, Vec3f velocity);
...
protected:
	...
	//particles and forces. use vector so that I can push
	vector<particle> particles;
	force** forces;
	int no_of_forces;

	//vector baked particles
	vector<baked_particles> b_particles;
